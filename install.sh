...
NIGHTLY_PLIST_SRC="$ROOT/launchd/ai.openclaw.circleback-gbrain-nightly.plist"
NIGHTLY_PLIST_DST="$HOME/Library/LaunchAgents/ai.openclaw.circleback-gbrain-nightly.plist"
NODE_BIN="$(command -v node)"

mkdir -p "$HOME/brain/meetings/circleback" "$HOME/brain/raw/circleback" "$HOME/Library/LaunchAgents" "$HOME/Library/Logs/gbrain-openclaw"
...
launchctl bootout "gui/$UID" "$NIGHTLY_PLIST_DST" >/dev/null 2>&1 || true
launchctl bootstrap "gui/$UID" "$NIGHTLY_PLIST_DST"
...
echo "Nightly maintenance: 03:17 local time"
